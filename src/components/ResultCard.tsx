...
export default function ResultCard({
  ...
  return (
    <div>
      <div
        className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-x-4 items-center ml-3 px-4 py-2 border-b hover:bg-gray-50 text-sm relative"
        onDoubleClick={() => onDoubleClick?.(fifo)}
      >
        ...
        <div className="text-gray-600 truncate">
          {user?._id === currentUser?._id ? "Tôi" : user?.user_name}
        </div>

        {/* Sửa đổi lần cuối */}
        <div className="text-gray-600 truncate">
          {option === "owner" || option === "shared"
            ? fifo.last_modified
              ? new Date(fifo.last_modified).toLocaleDateString()
              : "-"
            : fifo.deleted_at
            ? new Date(fifo.deleted_at).toLocaleDateString()
            : "-"}
        </div>
        ...
        <div>
          <span className="font-medium text-black truncate px-4">
            {isFile(fifo) ? fifo.document_category : "-"}
          </span>
        </div>
        ...
        <div className="relative justify-self-end" ref={menuRef}>
          ...
          {menuOpen && (option === "owner" || option === "shared") && (
            <div className="absolute right-0 mt-2 w-60 bg-white shadow-md rounded z-10">
              ...
              <button
                className="flex items-center px-4 py-2 w-full hover:bg-gray-100"
                onClick={() => setShowChangeCategory(!showChangeCategory)}
              >
          ...
          {menuOpen && option === "trash" && (
            <div className="absolute right-0 mt-2 w-60 bg-white shadow-md rounded z-10">
              ...
              <button
                className="flex items-center px-4 py-2 w-full text-red-500 hover:bg-gray-100"
