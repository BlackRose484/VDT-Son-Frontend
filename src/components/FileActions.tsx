...
import {
  completeFolderUploadAPI,
  prepareFolderUploadAPI,
  createFolderAPI,
...
export default function FileActions() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const end = Date.now();
        console.log(
          `✅ Successfully uploaded ${file.name} in ${end - start}ms`
        );
      } catch (error) {
  ...
  const handleFolderUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log(
        `[2/4] Received ${filesToUpload.length} file URLs. Starting parallel upload...`
      );
      ...
      const uploadPromises = filesToUpload.map((fileToUpload: any) => {
        ...
        if (!file) {
          console.error(
            `Could not find file for path: ${fileToUpload.relativePath}`
          );
        ...
        return uploadFileToS3(fileToUpload.uploadUrl, file).then((response) => {
          if (!response.ok)
      ...
      console.log(
        `[3/4] All ${completedFiles.length} files uploaded to S3 successfully.`
      );
      ...
      console.error(
        "❌ An error occurred during the folder upload process:",
        error
      );
